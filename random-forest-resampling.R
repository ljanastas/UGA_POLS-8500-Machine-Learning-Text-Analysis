...
# For over/under sampling you can just use  the "ROSE" package
# For synthetic sampling you need to use the SMOTE package
library(ROSE)

ovuntraining = ovun.sample(trainY~., data=traindata, method="both")$data
...
rf_fit<-ranger(factor(trainY)~., data=ovuntraining, 
               importance='impurity',
               write.forest=TRUE,
               probability=TRUE)
...
smoteddata = SMOTE(as.factor(trainY)~., traindata)
